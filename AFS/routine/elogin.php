...
/*            echo '<pre>';
            ...
            $query = "SELECT `session`,`email`,`password`,`dept` FROM `user` WHERE `email` = '$email' AND `password` = '$password'";
            ...
            // $_SESSION['post-data']['username']
              ...
              $row = mysqli_fetch_array($result);
                $_SESSION = $row;
                if ($_SESSION['dept'] == "IOT" && $_SESSION['session']=="2018-19" ) {
                  header("location: exam/exam_date.php");
                }
                elseif ($_SESSION['dept'] =="ICTE" && $_SESSION['session']=="2018-19" ) {
                  header("location: exam/exam_date.php");
                }
                elseif ($_SESSION['dept'] == "IOT" && $_SESSION['session']=="2019-20" ) {
                  header("location: exam/exam_date_19_20.php");
                }
                elseif ($_SESSION['dept'] =="ICTE" && $_SESSION['session']=="2019-20" ) {
                  header("location: exam/exam_date_19_20.php");
                }
              }
          }
        ...
        <!--<div class="text-center">
          <a class="d-block small mt-3" href="updatepass.php">Update Your Account Password</a>
        </div>-->
      </div>
    </div>
  </div>
